...
if(isset($_SESSION['mdp'])) {
    if (isset($_POST['submitSuppComp'])) {
        ...
        if (isset($_POST['SuppComp'])) {
            ...
            unlink($dirfile);
            $rqt = $pdo->prepare('DELETE FROM experience_pro WHERE id_exp_pro = ?');
            $rqt->execute(array($idComp));
    ...
    if (isset($_POST['submitModComp'])) {
        ...
        if (isset($_POST['modifCompName'])) {
            ...
            if (isset($_POST['modifCompTitre'])) {
                $q = $pdo->prepare('SELECT libelle FROM experience_pro WHERE id_exp_pro = ?');
                ...
                $newNameImage = mb_strtolower('../Logo/' . $newNameImage . '.png');
                $newDBNameImage = mb_strtolower('Logo/' . $newName . '.png');
                rename($oldNameImage, $newNameImage);
                $rqt = $pdo->prepare('UPDATE experience_pro SET libelle = ?, logo = ? WHERE id_exp_pro = ?');
            ...
            if (isset($_POST['modifCompAlt'])) {
                ...
                $rqt = $pdo->prepare('UPDATE experience_pro SET alt = ? WHERE id_exp_pro = ?');
                $rqt->execute(array($alt, $idComp));
                $messageModAlt = 'Ok, alt de l\'image défini sur ' . $alt . '<br>';
            }
            if (isset($_POST['modifCompType'])) {
                $q = $pdo->prepare('SELECT m.contenu, m.titre FROM modaux m JOIN experience_pro p ON m.titre=p.libelle WHERE p.id_exp_pro = ?');
                $q->execute(array($idComp));
                $c = $q->fetch();
                $oldName = $c['titre'];
            ...
            if (isset($_POST['modifCompModal'])) {
                $q = $pdo->prepare('SELECT m.titre FROM modaux m JOIN experience_pro p ON m.titre=p.libelle WHERE p.id_exp_pro = ?');
                $q->execute(array($idComp));
                $c = $q->fetch();
                $oldName = $c['titre'];
                ...
                $rqt2 = $pdo->prepare('UPDATE experience_pro SET data_target = ?  WHERE id_exp_pro = ?');
    ...
    if (isset($_POST['submitAddComp'])) {
        ...
        if (isset($_POST['AddCompTitre']) && isset($_POST['AddCompAlt']) && isset($_POST['AddCompModal']) && isset($_POST['AddCompType']) && isset($_FILES['AddCompImg']['name'])) {
            ...
            $rqt = $pdo->prepare('SELECT * FROM experience_pro WHERE libelle = ?');
            $rqt->execute(array($nomComp));
            $compExists = $rqt->rowcount();
            if ($compExists == 0) {
                if (move_uploaded_file($_FILES['AddCompImg']['tmp_name'], $uploadfile)) {
                    $rqt = $pdo->prepare('INSERT INTO experience_pro(`libelle`, `alt`, `logo`, `data_target`) VALUES (?, ?, ?, ?)');
