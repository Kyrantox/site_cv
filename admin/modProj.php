...
if(isset($_SESSION['mdp'])) {
    if (isset($_POST['submitSuppComp'])) {
        ...
        if (isset($_POST['SuppComp'])) {
            ...
            $rqt = $pdo->prepare('SELECT libelle FROM projet WHERE id_projet = ?');
            ...
            unlink($dirfile);
            $rqt = $pdo->prepare('DELETE FROM projet WHERE id_projet = ?');
            $rqt->execute(array($idComp));
    ...
    if (isset($_POST['submitModComp'])) {
        ...
        if (isset($_POST['modifCompName'])) {
            ...
            if (isset($_POST['modifCompTitre'])) {
                $q = $pdo->prepare('SELECT libelle FROM projet WHERE id_projet = ?');
                ...
                $rqt = $pdo->prepare('UPDATE projet SET libelle = ?, logo = ? WHERE id_projet = ?');
            ...
            if (isset($_POST['modifCompAlt'])) {
                ...
                $rqt = $pdo->prepare('UPDATE projet SET alt = ? WHERE id_projet = ?');
                $rqt->execute(array($alt, $idComp));
                $messageModAlt = 'Ok, alt de l\'image défini sur ' . $alt . '<br>';
            }
            if (isset($_POST['modifCompType'])) {
                $q = $pdo->prepare('SELECT m.contenu, m.titre FROM modaux m JOIN projet p ON m.titre=p.libelle WHERE p.id_projet = ?');
                $q->execute(array($idComp));
                $c = $q->fetch();
                $oldName = $c['titre'];
            ...
            if (isset($_POST['modifCompModal'])) {
                $q = $pdo->prepare('SELECT m.titre FROM modaux m JOIN projet p ON m.titre=p.libelle WHERE p.id_projet = ?');
                $q->execute(array($idComp));
                $c = $q->fetch();
                $oldName = $c['titre'];
                ...
                $rqt2 = $pdo->prepare('UPDATE projet SET data_target = ?  WHERE id_projet = ?');
    ...
    if (isset($_POST['submitAddComp'])) {
        ...
        if (isset($_POST['AddCompTitre']) && isset($_POST['AddCompAlt']) && isset($_POST['AddCompModal']) && isset($_POST['AddCompType']) && isset($_FILES['AddCompImg']['name'])) {
            ...
            $rqt = $pdo->prepare('SELECT * FROM projet WHERE libelle = ?');
            $rqt->execute(array($nomComp));
            $compExists = $rqt->rowcount();
            if ($compExists == 0) {
                if (move_uploaded_file($_FILES['AddCompImg']['tmp_name'], $uploadfile)) {
                    $rqt = $pdo->prepare('INSERT INTO projet(`libelle`, `alt`, `logo`, `data_target`) VALUES (?, ?, ?, ?)');
